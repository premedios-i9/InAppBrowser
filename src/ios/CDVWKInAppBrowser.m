...
- (NSString *)getRedirectURIFromQuery:(NSString *)queryString
{
    // Regular expression to find the redirect_uri
    NSString *pattern = @"redirect_uri=([^&]+)";
    ...
    if (match) {
        NSRange matchRange = [match rangeAtIndex:1]; // Get the captured group (redirect_uri value)
        NSString *redirectUri = [queryString substringWithRange:matchRange];
        return redirectUri;
    } else {
        return @"";
    }
...
- (void)webView:(WKWebView *)theWebView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if (![url.absoluteString containsString:@".pdf"]) {
        self.inAppBrowserViewController.toolbar.hidden = true;
        CDVInAppBrowserOptions *options = [self.inAppBrowserViewController getBrowserOptions];
        options.toolbar = false;
        [self.inAppBrowserViewController setBrowserOptions:options];
        [self.inAppBrowserViewController rePositionViews];
    } else {
        self.inAppBrowserViewController.toolbar.hidden = false;
        CDVInAppBrowserOptions *options = [self.inAppBrowserViewController getBrowserOptions];
    ...
    if ([self.redirectURL isEqual:@""] || self.redirectURL == nil) {
        self.redirectURL = [self getRedirectURIFromQuery:url.query];
    } else if ([url.absoluteString hasPrefix:self.redirectURL] || [url.absoluteString containsString:@"close=true"]) {
...
- (void) setBrowserOptions:(CDVInAppBrowserOptions *)options {
...
- (void)close
{
    if ([self.currentURL.absoluteString containsString:@".pdf"]) {
        [self.webView goBack];
    } else {
        self.currentURL = nil;
